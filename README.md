...
>
> 包名 `com.localagent.plaza` · versionName `1.0.13-plaza.1` · 当前稳定版 v1.0.0-plaza.1

---

## 它是什么

Local Agent Plaza 是一款把「本地大模型推理」变成日常可用的 Android 应用。所有模型权重、对话历史、附件与工具结果都保存在设备本地，**不上传任何数据到云端**。

它是 Google AI Edge Gallery v1.0.13 的二次开发版本，移除了开发测试痕迹，重塑了产品命名与包名，但完整保留了上游的全部能力并补齐了无障碍侧的关键模块。

## 核心能力

- **本地 LLM 对话**：导入 GGUF / HuggingFace 模型后即可离线推理，支持 CPU / GPU / NPU 多后端切换
...
前往 [Releases](https://github.com/zhangzheyuanviolin-ship-it/gallery/releases) 下载最新的 `local-agent-plaza-*.apk`。

```bash
adb push local-agent-plaza-1.0.0.apk /data/local/tmp/
adb shell pm install -t /data/local/tmp/local-agent-plaza-1.0.0.apk
```

...
old='exec bash "$FIXED"\n'
new=r'''python3 - "$FIXED" <<'PY250MATERIALIZE'
from pathlib import Path
import sys
p=Path(sys.argv[1])
s=p.read_text()

# Apply MCP250 only after the complete MCP249 protected product line has been materialized.
anchor='python3 .github/scripts/patch_mcp249_ministral_phi_compat_v2.py\n'
insert=anchor+r'''export MCP250_PROTECTED_SNAPSHOT="$RUNNER_TEMP/mcp250_protected_core.json"
...
PY250PROTECTAFTER
'''
if s.count(anchor)!=1: raise SystemExit(f'MCP250 injection anchor count={s.count(anchor)}')
s=s.replace(anchor,insert,1)

# MCP250 adds one exact-policy source and one UI loop-hook source. Existing MCP249 files remain in
# the inherited expected delta.
...
if s.count(old_line)!=1: raise SystemExit(f'MCP250 expected-delta anchor count={s.count(old_line)}')
s=s.replace(old_line,new_line,1)

# APK DEX must contain target-only markers and both protected model names, proving exact-name
# isolation reached the packaged runtime.
...
if s.count(dex_old)!=1: raise SystemExit(f'MCP250 DEX anchor count={s.count(dex_old)}')
...
repls={
  ...
  'docs/mcp249_run.json':'docs/mcp250_run.json',
  "'Update MCP249 run marker'":"'Update MCP250 run marker'",
}
for old,new in repls.items():
    c=s.count(old)
    if c<1: raise SystemExit(f'MCP250 materialize missing {old!r}')
    s=s.replace(old,new)
...
if s.count(manifest_anchor)!=1: raise SystemExit(f'MCP250 manifest anchor count={s.count(manifest_anchor)}')
s=s.replace(manifest_anchor,manifest_insert,1)

# Final source-level fail-closed guard: protected baselines must still use the original non-target
# parser branch and must be explicitly excluded by exact target classification.
pre_build_anchor="echo MCP249_EXACT_MCP248_MODEL_POOL_IDENTITY_PASS\n"
pre_build_insert=pre_build_anchor+r'''grep -F 'PROTECTED_LOCO = "LocoOperator-4B LiteRTLM"' Android/src/app/src/main/java/com/google/ai/edge/gallery/customtasks/agentchat/Mcp250TargetAgentCompat.kt
...
'''
if s.count(pre_build_anchor)!=1: raise SystemExit(f'MCP250 protected static anchor count={s.count(pre_build_anchor)}')
...
chmod +x "$WRAPPED"
bash -n "$WRAPPED"
grep -F 'PY250MATERIALIZE' "$WRAPPED"
grep -F 'MCP250_PROTECTED_CORE_ROUTING_STATIC_PASS' "$WRAPPED"
exec bash "$WRAPPED"
